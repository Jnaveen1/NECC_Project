...
export default function Dashboard({ locations, filters, onFilterChange, onApply, summary, daily, loading, marketInsight, insightLoading }) {
  ...
  useEffect(() => {
        ...
        const [monthResponses, dayResponses] = await Promise.all([
          Promise.all(
            years.map(async (year) => {
              try {
                return await fetchMonthlySummary({ location: filters.location, year });
              } catch {
                return [];
              }
            })
          ...
          Promise.all(
            years.map(async (year) => {
              try {
                ...
                return await fetchDailyPrices({ location: filters.location, startDate: toIsoDate(start), endDate: toIsoDate(end) });
              } catch {
                return [];
              }
            })
    ...
    if (filters.location) loadComparison();
    return () => { active = false; };
  }, [filters.location, years]);
    ...
    <>
      <section className="panel comparison-panel">
        <div className="panel-head">
          <div><h2>Last 3 years comparison</h2><p>Compare {filters.location} across the latest three years</p></div>
